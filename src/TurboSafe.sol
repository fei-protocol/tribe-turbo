...
contract TurboSafe is Auth, ERC20, ERC4626 {
        ...
        )
    {
        ...
        fei = master.fei();
        pool = master.pool();
        feiCToken = pool.cTokensByUnderlying(fei);
    ...
    function slurp(ERC4626 vault) public {
        require(getTotalFeiBoosted[vault] > 0, "NO_FEI_BOOSTED");

        // Compute the amount of Fei interest the Safe generated by boosting the vault.
        uint256 interestEarned = vault.balanceOfUnderlying(address(this)) - getTotalFeiBoosted[vault];
        ...
        getTotalFeiBoosted[vault] += safeInterestAmount;
